...
const FEATURES = [
    ...
    {
        title: "Resume Analyzer", icon: "📊", path: "skill-matcher",
        desc: "Get a real-time ATS score and line-by-line feedback to maximise your interview chances.",
        iconBg: "rgba(134,239,172,0.1)", iconBorder: "rgba(134,239,172,0.2)",
        btnBg: "rgba(134,239,172,0.1)", btnBorder: "rgba(134,239,172,0.25)", btnColor: "#86efac",
        topLine: "linear-gradient(90deg,transparent,#86efac,transparent)",
    },
...
function buildStatCards(stats) {
    ...
    return [
        ...
        {
            title: "Improvement",
            value: stats.improvement_pct != null ? `${deltaSign(stats.improvement_pct)}%` : "—",
            delta: stats.improvement_pct != null ? "Since last analysis" : "Analyse to track",
            positive: (stats.improvement_pct ?? 0) >= 0,
            icon: "📈",
            iconBg: "rgba(249,168,212,0.1)", iconBorder: "rgba(249,168,212,0.2)",
            accent: "#f9a8d4", cardBorder: "rgba(249,168,212,0.2)",
            cardGrad: "linear-gradient(135deg,rgba(249,168,212,0.08),rgba(249,168,212,0.01))",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {aiTip === undefined ? (
                ...
                <div style={{
                    ...
                    <button
                        onClick={() => onFeatureClick(aiTip.action_path)}
                        style={{
                            marginLeft: "auto", flexShrink: 0, fontSize: 12,
                            padding: "5px 13px", borderRadius: 999,
                            background: "rgba(232,255,71,0.1)",
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14 }}>
                {FEATURES.map((f) => (
                    <FeatureCard key={f.title} {...f} onClick={() => onFeatureClick(f.path)} />
                ))}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                ...
                >
                    ...
                    {activity === null ? (
                        ...
                        <div style={{ padding: "24px 20px", textAlign: "center", fontSize: 13, color: t.faint }}>
                        ...
                        activity.map((a, i) => (
                            ...
                            >
                                ...
                                <span style={{ flex: 1, fontSize: 13, color: t.muted }}>{a.label}</span>
                                <span style={{ fontSize: 11, color: t.faint, flexShrink: 0 }}>{a.time}</span>
                                <span style={{ width: 6, height: 6, borderRadius: "50%", background: ACTIVITY_DOTS[a.action_type] ?? t.faint, flexShrink: 0 }} />
                            </div>
                        ))
                    )}
                </div>
                        ...
                        {score === undefined ? (
                            <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
                                ...
                                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10 }}>
                                    {Array(4).fill(null).map((_, i) => <Skeleton key={i} h={3} radius={2} />)}
                                </div>
                            </div>
                        ) : !score ? (
                            <div style={{ fontSize: 12, color: t.faint, textAlign: "center", padding: "12px 0" }}>
                                No score yet — analyse your resume to see a breakdown.
                            </div>
                        ) : (
                            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                                <ScoreRing score={score.overall} size={80} stroke={6} />
                                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 7 }}>
                                    {SCORE_BAR_CONFIG.map((b) => (
